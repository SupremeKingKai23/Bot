...
public class SmogonSetList
{
    ...
    {
        if (string.IsNullOrWhiteSpace(form) || IsInvalidForm(form))
            return species;
        return $"{species}-{form}";
    }
    ...
    public List<string> GetTitles()
    {
        ...
        for (int i = 0; i < Sets.Count; i++)
        {
            ...
            var name = SetName[i];
            var formattedTitle = $"{format}: {name}";
    ...
    public static List<string> GetTitles(PKM pk)
    {
        SmogonSetList smogonSetList = new(pk);
        return smogonSetList.GetTitles();
    }
    ...
    public static List<ShowdownSet>? GenerateSets(PKM rough)
    {
        try
        {
            var info = new SmogonSetList(rough);
            if (!info.Valid || info.Sets.Count == 0) return null;

            var setsToReturn = new List<ShowdownSet>();
            while (info.Sets.Count > 0)
            {
                setsToReturn.Add(info.Sets[0]);
                info.Sets.RemoveAt(0);
                info.SetFormat.RemoveAt(0);
                info.SetName.RemoveAt(0);
                info.SetConfig.RemoveAt(0);
                info.SetText.RemoveAt(0);
            }
            return setsToReturn;
        }
        catch
