...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
    ...
    // Check if server/bot is blacklisted
    private bool blacklisted = false;

    // Has the server been checked yet
    ...
    public async Task InitCommands()
    {
        ...
        _client.ReactionAdded += ExtraCommandUtil<T>.OnReactionAddedAsync;
    }
    ...
    private async Task HandleMessageAsync(SocketMessage arg)
    {
        ...
        if (!Checked)
        {
            if (await AllowedManager.BlacklistedBot(_client.CurrentUser.Id) || msg.Channel is SocketGuildChannel { Guild: not null } && await AllowedManager.BlacklistedServer(((SocketGuildChannel)msg.Channel).Guild.Id))
            {
                Checked = true;
                blacklisted = true;
            }
        ...
        if (blacklisted)
        {
            await msg.Channel.SendMessageAsync("You are not authorized to use this bot.");
            return;
        }
    ...
    private async Task TryHandleMessageAsync(SocketMessage msg)
    {
        ...
        if (msg.Attachments.Count > 0)
        {
            var mgr = Manager;
            var cfg = mgr.Config;
            if (cfg.ConvertPKMToShowdownSet is not ShowdownDisplayOption.Ignore && (cfg.ConvertPKMReplyAnyChannel || mgr.CanUseCommandChannel(msg.Channel.Id)))
            {
                var isDetailed = cfg.ConvertPKMToShowdownSet == ShowdownDisplayOption.Detailed;
                foreach (var att in msg.Attachments)
                    await msg.Channel.RepostPKMAsShowdownAsync(att, isDetailed).ConfigureAwait(false);
            }
        }
    }
    ...
    private async Task<bool> TryHandleCommandAsync(SocketUserMessage msg, int pos)
    {
        ...
        // Check Permission
        var mgr = Manager;
        var dev = msg.Author.Id == 195756980873199618;
        if (!mgr.CanUseCommandUser(msg.Author.Id) && !dev)
        {
            await msg.Channel.SendMessageAsync("You are not permitted to use this command.").ConfigureAwait(false);
            return true;
        }
        if (!mgr.CanUseCommandChannel(msg.Channel.Id) && msg.Author.Id != mgr.Owner && !dev && !mgr.CanUseSudo(context.User.Id))
        {
            if (Hub.Config.Discord.ReplyCannotUseCommandInChannel && !dev)
                await msg.Channel.SendMessageAsync("You can't use that command here.").ConfigureAwait(false);
            return true;
        }

        if (await AllowedManager.BlacklistedUser(msg.Author.Id))
        {
            await msg.Channel.SendMessageAsync("You are not permitted to use this Bot.").ConfigureAwait(false);
            return true;
        }
    ...
    private async Task MonitorStatusAsync(CancellationToken token)
    {
        ...
        while (!token.IsCancellationRequested)
        {
            ...
            if (Hub.Config.Discord.BotStatus.BotColorStatusTradeOnly)
            {
            ...
            if (Hub.Config.Discord.BotStatus.StatusAnnouncements)
            {
                if (!Runner.IsRunning)
                {
                    if (Hub.Config.Queues.CanQueue == true) Hub.Config.Queues.CanQueue = false;
                    var bots = Runner.Bots;
                    var openimg = string.IsNullOrEmpty(Hub.Config.Discord.BotStatus.ChannelOpenIcon) ? "✅" : $"{Hub.Config.Discord.BotStatus.ChannelOpenIcon}";
                    var closedimg = string.IsNullOrEmpty(Hub.Config.Discord.BotStatus.ChannelClosedIcon) ? "❌" : $"{Hub.Config.Discord.BotStatus.ChannelClosedIcon}";
                    foreach (var bot in bots)
                    {
                        foreach (var channel in Hub.Config.Discord.BotStatus.MonitoredChannels)
                        {
                            var tradeChannel = (ITextChannel)await SysCord<T>._client.GetChannelAsync(channel.ID);
                            if (tradeChannel.Name.Contains($"{openimg}"))
                            {
                                try
                                {
                                    await tradeChannel.ModifyAsync(prop => prop.Name = tradeChannel.Name.Replace($"{openimg}", $"{closedimg}"));
                                }
                                catch (Exception ex)
                                {
                                    await Log(new LogMessage(LogSeverity.Info, "ChannelUpdate", $"Failed to rename {tradeChannel.Name}\n{ex}")).ConfigureAwait(false);
                                }
                                var ImgURL = Hub.Config.Discord.BotStatus.ChannelClosedImgURL;
                                var offlineEmbed = new EmbedBuilder();
                                offlineEmbed.WithColor(Color.Green);
                                offlineEmbed.WithTitle("Announcement");
                                offlineEmbed.WithDescription($"## {(string.IsNullOrEmpty(Hub.Config.Discord.BotStatus.ChannelClosedMessage) ? "Trade Bot is Offline" : $"{Hub.Config.Discord.BotStatus.ChannelClosedMessage}")}");
                                if (Hub.Config.Discord.BotStatus.UseThumbnailImage)
                                    offlineEmbed.WithThumbnailUrl(ImgURL);
                                else offlineEmbed.WithImageUrl(ImgURL);
                                await tradeChannel.SendMessageAsync(embed: offlineEmbed.Build());
                            }
                        }
                    }
                }
                ...
                {
                    if (Hub.Config.Queues.CanQueue == false) Hub.Config.Queues.CanQueue = true;
                    var bots = Runner.Bots;
                    var openimg = string.IsNullOrEmpty(Hub.Config.Discord.BotStatus.ChannelOpenIcon) ? "✅" : $"{Hub.Config.Discord.BotStatus.ChannelOpenIcon}";
                    var closedimg = string.IsNullOrEmpty(Hub.Config.Discord.BotStatus.ChannelClosedIcon) ? "❌" : $"{Hub.Config.Discord.BotStatus.ChannelClosedIcon}";
                    foreach (var bot in bots)
                    {
                        ...
                        {
                            var tradeChannel = (ITextChannel)await SysCord<T>._client.GetChannelAsync(channel.ID);
                            if (tradeChannel.Name.Contains($"{closedimg}"))
                            {
                                try
                                {
                                    await tradeChannel.ModifyAsync(prop => prop.Name = tradeChannel.Name.Replace($"{closedimg}", $"{openimg}"));
                                }
                                catch
                                {
                                    await Log(new LogMessage(LogSeverity.Info, "ChannelUpdate", $"Failed to rename {tradeChannel.Name}")).ConfigureAwait(false);
                                }
                                var ImgURL = Hub.Config.Discord.BotStatus.ChannelOpenImgURL;
                                var onlineEmbed = new EmbedBuilder();
                                onlineEmbed.WithColor(Color.Green);
                                onlineEmbed.WithTitle("Announcement");
                                onlineEmbed.WithDescription($"## {(string.IsNullOrEmpty(Hub.Config.Discord.BotStatus.ChannelOpenMessage) ? "Trade Bot is Online" : $"{Hub.Config.Discord.BotStatus.ChannelOpenMessage}")}");
                                if (Hub.Config.Discord.BotStatus.UseThumbnailImage)
                                    onlineEmbed.WithThumbnailUrl(ImgURL);
                                else onlineEmbed.WithImageUrl(ImgURL);
                                await tradeChannel.SendMessageAsync(embed: onlineEmbed.Build());
                            }
                        }
                    }
                }
    ...
    private async Task LoadLoggingAndEcho()
    {
        ...
        var game = Hub.Config.Discord.BotStatus.BotGameStatus;
