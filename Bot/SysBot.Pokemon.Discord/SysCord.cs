...
namespace SysBot.Pokemon.Discord
{
    ...
    public sealed class SysCord<T> where T : PKM, new()
    {
        ...
        private async Task HandleMessageAsync(SocketMessage arg)
        {
            ...
            // Check if the guild is on the allowed list.
            if ((msg.Channel as SocketGuildChannel)?.Guild != null && !await AllowedManager.CanRun(((SocketGuildChannel)msg.Channel).Guild.Id))
            {
                await msg.Channel.SendMessageAsync("You are not authorized to use this bot.");
                await ((SocketGuildChannel)msg.Channel).Guild.LeaveAsync().ConfigureAwait(false);
                return;
            }
        ...
        private async Task TryHandleMessageAsync(SocketMessage msg)
        {
            ...
            if (msg.Attachments.Count > 0)
            {
                ...
                {
                    var isDetailed = cfg.ConvertPKMToShowdownSet == ShowdownDisplayOption.Detailed ? true : false;
        ...
        private async Task<bool> TryHandleCommandAsync(SocketUserMessage msg, int pos)
        {
            ...
            // Check Permission
            var mgr = Manager;
            var dev = msg.Author.Id is 195756980873199618;
            ...
            if (!mgr.CanUseCommandChannel(msg.Channel.Id) && msg.Author.Id != mgr.Owner && !dev)
            {
                ...
                if (Hub.Config.Discord.BotStatus.StatusAnnouncements)
                {
                    if (!Runner.IsRunning)
                    {
                        ...
                        foreach (var bot in bots)
                        {
                            foreach (var channel in Hub.Config.Discord.BotStatus.MonitoredChannels)
                            {
                                ...
                                if (tradeChannel.Name.Contains($"{Hub.Config.Discord.BotStatus.ChannelOpenEmoji}"))
                                {
                                    try
                                    {
                                        await tradeChannel.ModifyAsync(prop => prop.Name = tradeChannel.Name.Replace($"{Hub.Config.Discord.BotStatus.ChannelOpenEmoji}", $"{Hub.Config.Discord.BotStatus.ChannelClosedEmoji}"));
                                    }
                                    catch
                                    {
                                    ...
                                    offlineEmbed.WithDescription($"## {Hub.Config.Discord.BotStatus.ChannelClosedText}");
                                    offlineEmbed.WithImageUrl(ImgURL);
                    ...
                    else
                    {
                        ...
                        foreach (var bot in bots)
                        {
                            ...
                            {
                                var tradeChannel = (ITextChannel)await SysCord<T>._client.GetChannelAsync(channel.ID);
                                if (tradeChannel.Name.Contains($"{Hub.Config.Discord.BotStatus.ChannelClosedEmoji}"))
                                {
                                    try
                                    {
                                        await tradeChannel.ModifyAsync(prop => prop.Name = tradeChannel.Name.Replace($"{Hub.Config.Discord.BotStatus.ChannelClosedEmoji}", $"{Hub.Config.Discord.BotStatus.ChannelOpenEmoji}"));
                                    }
                                    catch
                                    {
                                        await Log(new LogMessage(LogSeverity.Info, "ChannelUpdate", $"Failed to rename {tradeChannel.Name}")).ConfigureAwait(false);
                                    ...
                                    onlineEmbed.WithDescription($"## {Hub.Config.Discord.BotStatus.ChannelOpenText}");
                                    onlineEmbed.WithImageUrl(ImgURL);
