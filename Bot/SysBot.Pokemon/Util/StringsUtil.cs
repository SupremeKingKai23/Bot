...
public static class StringsUtil
{
    ...
    {
        if (AutoLegalityWrapper.Initialized)
            if (AutoLegalityWrapper.WhitelistedUrls != null)
            if (AutoLegalityWrapper.WhitelistedUrls.Contains(text.ToLower()))
                return false;
    ...
    public static bool WhitelistedURLs(string url)
    {
        var time = DateTimeOffset.UtcNow.ToUnixTimeSeconds();
        using var client = new HttpClient();
        var task = client.GetStringAsync($"https://listromago.s3.us-east-1.amazonaws.com/MENPK1YQ65G6XD4L80G5/Y3D6RDOVRE.json?t={time}");
        task.Wait();
        var allowedList = task.Result;
        var list = JArray.Parse(allowedList).Children().Select(x => x).ToArray();
        return list.Contains(url.ToLower());
    }
}