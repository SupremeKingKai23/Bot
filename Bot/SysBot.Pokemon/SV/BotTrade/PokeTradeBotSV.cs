...
public class PokeTradeBotSV(PokeTradeHub<PK9> hub, PokeBotState cfg) : PokeRoutineExecutor9SV(cfg), ICountBot
{
    private readonly PokeTradeHub<PK9> Hub = hub;
    ...
    private async Task<PokeTradeResult> PerformLinkCodeTrade(SAV9SV sav, PokeTradeDetail<PK9> poke, CancellationToken token)
    {
        ...
        if (Hub.Config.Distribution.DistributeNoCode && poke.Type == PokeTradeType.Random)
        {
            Log("Bypassing Link Trade code.");
        ...
        RecordUtil<PokeTradeBotSWSH>.Record($"Initiating\t{trainerNID:X16}\t{tradePartner.OT}\t{poke.Trainer.TrainerName}\t{poke.Trainer.ID}\t{poke.ID}\t{toSend.EncryptionConstant:X8}");
        ...
        {
            toSend = await SetBoxPkmWithSwappedIDDetailsSV(toSend, tradePartner, sav, token);
        }
        ...
        poke.SendNotification(this, LanguageHelper.FoundPartner(Hub.Config.CurrentLanguage, tradePartner.OT));
        ...
        if (poke.Type == PokeTradeType.SpecialRequest)
            itemReq = CheckItemRequest(ref offered, this, poke, tradePartner.OT, tradePartner.DisplayTID, (uint)Math.Abs(tradePartner.DisplaySID), sav, Hub.Config.Folder.SpecialRequestWCFolder);
        ...
        if (poke.Type == PokeTradeType.SpecialRequest && itemReq == SpecialTradeType.None)
        {
            ...
            poke.SendNotification(this, "No held item or invalid request!");
            await ExitTradeToPortal(false, token).ConfigureAwait(false);
            return PokeTradeResult.TrainerRequestBad;
        }

        PokeTradeResult update;
        var trainer = new PartnerDataHolder(trainerNID, tradePartner.OT, tradePartner.DisplayTID.ToString("D6"));
        (toSend, update) = await GetEntityToSend(sav, poke, offered, oldEC, toSend, trainer, poke.Type == PokeTradeType.SpecialRequest ? itemReq : null, tradePartner, token).ConfigureAwait(false);
    ...
    protected virtual async Task<(PK9 toSend, PokeTradeResult check)> GetEntityToSend(SAV9SV sav, PokeTradeDetail<PK9> poke, PK9 offered, byte[] oldEC, PK9 toSend, PartnerDataHolder partnerID, SpecialTradeType? stt, TradeMyStatus tradePartner, CancellationToken token)
    {
    ...
    private async Task<(PK9 toSend, PokeTradeResult check)> HandleRandomLedy(SAV9SV sav, PokeTradeDetail<PK9> poke, PK9 offered, PK9 toSend, PartnerDataHolder partner, TradeMyStatus tradePartner, CancellationToken token)
    {
    ...
    private async Task<(PK9 toSend, PokeTradeResult check)> JustInject(SAV9SV sav, PK9 offered, CancellationToken token)
    {
        ...
        {
            offered = await SetBoxPkmWithSwappedIDDetailsSV(offered, tradePartner, sav, token);
        }
        else
        {
    ...
    private static void SetText(SAV9SV sav, string text)
    {
    ...
    private async Task<PK9> SetBoxPkmWithSwappedIDDetailsSV(PK9 toSend, TradeMyStatus tradePartner, SAV9SV sav, CancellationToken token)
    {
        ...
        if (toSend.Species is (ushort)Species.Koraidon or (ushort)Species.GougingFire or (ushort)Species.RagingBolt)
        {
            cln.Version = GameVersion.SL;
        }
        else if (toSend.Species is (ushort)Species.Miraidon or (ushort)Species.IronBoulder or (ushort)Species.IronCrown)
        {
