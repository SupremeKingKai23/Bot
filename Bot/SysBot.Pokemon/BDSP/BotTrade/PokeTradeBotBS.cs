...
public class PokeTradeBotBS(PokeTradeHub<PB8> hub, PokeBotState cfg) : PokeRoutineExecutor8BS(cfg), ICountBot
{
    private readonly PokeTradeHub<PB8> Hub = hub;
    ...
    private async Task<PokeTradeResult> PerformLinkCodeTrade(SAV8BS sav, PokeTradeDetail<PB8> poke, CancellationToken token)
    {
        ...
        if (Hub.Config.Trade.MiscSettings.CenterUnionRoom)
        {
            if (distroRemainInRoom & !set)
            {
                set = true;
                await CenterInUnionRoom(token).ConfigureAwait(false);
            }
            else if (!distroRemainInRoom)
            {
                set = false;
                await CenterInUnionRoom(token).ConfigureAwait(false);
            }
        ...
        poke.SendNotification(this, LanguageHelper.FoundPartner(Hub.Config.CurrentLanguage, tradePartner.TrainerName));
        ...
        if (Hub.Config.Legality.UseTradePartnerInfo && poke.Type != PokeTradeType.SpecialRequest && poke.Type != PokeTradeType.Random)
        {
            Log($"Changing OT to Partner OT");
            toSend = await SetBoxPkmWithSwappedIDDetailsBDSP(toSend, offered, sav, tradePartner, token);
            await Click(R, 1_750, token).ConfigureAwait(false);
            await Click(L, 1_750, token).ConfigureAwait(false);
        ...
        (toSend, update) = await GetEntityToSend(sav, poke, offered, oldEC, toSend, trainer, tradePartner, poke.Type == PokeTradeType.SpecialRequest ? itemReq : null, token).ConfigureAwait(false);
    ...
    private async Task<bool> EnterUnionRoomWithCode(PokeTradeType tradeType, int tradeCode, CancellationToken token)
    {
        ...
        if (tradeType == PokeTradeType.Random && Hub.Config.Distribution.DistributeNoCode)
        {
            Log("Selecting Yes.");
        }
        ...
        else if (tradeType == PokeTradeType.Random && Hub.Config.Distribution.DistributeNoCode)
        {
            WaitAtBarrierIfApplicable(token);
        }
    ...
    private async Task CenterInUnionRoom(CancellationToken token)
    {        
        Log("Moving to center of Union Room");
        await PressAndHold(DUP, 2500, 0, token);
    }
    ...
    protected virtual async Task<(PB8 toSend, PokeTradeResult check)> GetEntityToSend(SAV8BS sav, PokeTradeDetail<PB8> poke, PB8 offered, byte[] oldEC, PB8 toSend, PartnerDataHolder partnerID, TradePartnerBS tradePartner, SpecialTradeType? stt, CancellationToken token)
    {
    ...
    private async Task<(PB8 toSend, PokeTradeResult check)> HandleRandomLedy(SAV8BS sav, PokeTradeDetail<PB8> poke, PB8 offered, PB8 toSend, PartnerDataHolder partner, TradePartnerBS tradePartner, CancellationToken token)
    {
    ...
    private async Task<(PB8 toSend, PokeTradeResult check)> JustInject(SAV8BS sav, PB8 offered, CancellationToken token)
    {
        ...
        if (offered.OriginalTrainerName == sav.OT)
        {
            var tradePartner = await GetTradePartnerInfo(token).ConfigureAwait(false);
            Log($"Changing OT to Partner OT");
            offered = await SetBoxPkmWithSwappedIDDetailsBDSP(offered, offered, sav, tradePartner, token);
            await Click(R, 1_750, token).ConfigureAwait(false);
            await Click(L, 1_750, token).ConfigureAwait(false);
            await Task.Delay(1_500, token).ConfigureAwait(false);
        }
    ...
    private async Task<PB8> SetBoxPkmWithSwappedIDDetailsBDSP(PB8 toSend, PB8 offered, SAV8BS sav, TradePartnerBS tradePartner, CancellationToken token)
    {
