...
public class LegalitySettings
{
    ...
    [Category(Generate), Description("Iclude Legality Analysis in Legality report embeds upon failed Showdownset Import")]
    public bool IncludeLegalityAnalysis { get; set; } = false;
    ...
    [Category(Generate), Description("Set suggested moves for any generated Pokémon")]
    public bool SetSuggestedMoves { get; set; } = true;

    [Category(Generate), Description("Set suggested relearn moves for any generated Pokémon")]
    public bool SetSuggestedRelearnMoves { get; set; } = false;

    [Category(Generate), Description("Set all technical record moves as relearnable for any generated Pokémon")]
    public bool SetAllTechnicalRecords { get; set; } = false;

    [Category(Generate), Description("Force the specified ball if legal.")]
    public bool ForceSpecifiedBall { get; set; } = true;

    [Category(Generate), Description("If set to \"True\", anytime a user requests a Pokémon at level 50 it will automatically be set to level 100.")]
    ...
    [Category(Generate), Description("Allow user provided files with Home Trackers to be traded. Does NOT effect Distributions or giveaways")]
    public bool AllowHOMETrackers { get; set; } = false;
