...
{
    public class PokemonGAPool<T> : List<T> where T : PKM, new()
    {
        ...
        public PokemonGAPool(BaseConfig settings) => Settings = settings;
        ...
        public bool LoadFolder(string path, SearchOption opt = SearchOption.AllDirectories)
        {
            ...
            foreach (var file in matchFiles)
            {
                ...
                {
                    LogUtil.LogInfo("Provided file was loaded but can't be Surprise Traded: " + dest.FileName, nameof(PokemonGAPool<T>));
                    surpriseBlocked++;
                }
            ...
            {
                LogUtil.LogInfo("Provided pool to distribute has the same OT for all loaded. Pool is not valid; please distribute from a variety of trainers.", nameof(PokemonGAPool<T>));
                surpriseBlocked = Count;
                Files.Clear();
            }

            if (surpriseBlocked == Count)
                LogUtil.LogInfo("Surprise trading will fail; failed to load any compatible files.", nameof(PokemonGAPool<T>));
