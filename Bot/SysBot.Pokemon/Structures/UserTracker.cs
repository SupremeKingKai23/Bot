...
public class TrackedUserLog
{
    ...
    public readonly List<TrackedUser> Users = new(Capacity);
    ...
    public void RemoveAllName(string name)
    {
        lock (_sync)
            Users.RemoveAll(z => z.Name == name);
    }
    ...
    public TrackedUser? TryGetPreviousName(string name)
    {
        lock (_sync)
            return Users.Find(z => z.Name == name);
    }
    ...
    {
        lock (_sync)
            return Users.FindAll(z => z.NetworkID != 0).OrderBy(z => z.Name).Select(z => $"{z.Name}, NID: {z.NetworkID}, Discord ID: {z.RemoteID}").ToList();
    }
}
