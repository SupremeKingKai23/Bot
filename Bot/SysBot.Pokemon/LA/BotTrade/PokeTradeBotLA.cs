...
public class PokeTradeBotLA(PokeTradeHub<PA8> hub, PokeBotState cfg) : PokeRoutineExecutor8LA(cfg), ICountBot
{
    private readonly PokeTradeHub<PA8> Hub = hub;
    ...
    private async Task<PokeTradeResult> PerformLinkCodeTrade(SAV8LA sav, PokeTradeDetail<PA8> poke, CancellationToken token)
    {
        ...
        var trainer = new PartnerDataHolder(trainerNID, tradePartner.TrainerName, tradePartner.TID);
        ...
        {
            toSend = await SetBoxPkmWithSwappedIDDetailsPLA(toSend, tradePartner, sav, token);
        }
    ...
    private void UpdateCountsAndExport(PokeTradeDetail<PA8> poke, PA8 received, PA8 toSend)
    {
        ...
        if (DumpSetting.Dump && !string.IsNullOrEmpty(DumpSetting.DumpFolder))
        {
            ...
            if (poke.Type is PokeTradeType.Specific or PokeTradeType.Clone or PokeTradeType.FixOT or PokeTradeType.SpecialRequest)
    ...
    protected virtual async Task<(PA8 toSend, PokeTradeResult check)> GetEntityToSend(SAV8LA sav, PokeTradeDetail<PA8> poke, PA8 offered, byte[] oldEC, PA8 toSend, PartnerDataHolder partnerID, TradePartnerLA tradePartner, SpecialTradeType? stt, CancellationToken token)
    {
    ...
    private async Task<(PA8 toSend, PokeTradeResult check)> HandleRandomLedy(SAV8LA sav, PokeTradeDetail<PA8> poke, PA8 offered, PA8 toSend, PartnerDataHolder partner, TradePartnerLA tradePartner, CancellationToken token)
    {
    ...
    private async Task<(PA8 toSend, PokeTradeResult check)> JustInject(SAV8LA sav, PA8 offered, CancellationToken token)
    {
        ...
        {
            offered = await SetBoxPkmWithSwappedIDDetailsPLA(offered, tradePartner, sav, token);
        }
        else
        {
    ...
    private static void SetText(SAV8LA sav, string text)
    {
    ...
    private async Task<PA8> SetBoxPkmWithSwappedIDDetailsPLA(PA8 toSend, TradePartnerLA tradePartner, SAV8LA sav, CancellationToken token)
    {
