...
public static class SpecialRequests
{
    ...
    public static SpecialTradeType CheckItemRequest<T>(ref T pk, PokeRoutineExecutor<T> caller, PokeTradeDetail<T> detail, string TrainerName, uint TID, uint SID, SaveFile sav, string folder) where T : PKM, new()
    {
                ...
                if (typeof(T) == typeof(PK8))
                {
                    ...
                    else
                    {
                        try
                        {
                            ...
                            string[] MaxLairLegendaries = ["144", "145", "146", "150", "243", "244", "245", "249", "250", "380", "381", "382", "383", "384", "480", "481", "482", "483", "484", "485", "487", "488", "641", "642", "643", "644", "645", "646", "716", "717", "718", "785", "786", "787", "788", "791", "792", "793", "794", "795", "796", "797", "798", "799", "800", "805", "806"];
        ...
        else if (typeof(T) == typeof(PK9) && ItemStorage9SV.GetInventoryPouch((ushort)pk.HeldItem) is InventoryType.TMHMs || typeof(T) == typeof(PK8) && ItemStorage8SWSH.IsTechRecord((ushort)pk.HeldItem) || typeof(T) == typeof(PB8) && ItemStorage8BDSP.GetInventoryPouch((ushort)pk.HeldItem) is InventoryType.TMHMs)
        {
            if (pk is ITechRecord t)
                t.SetRecordFlagsAll();
            pk.HeldItem = heldItemNew; //free master
        ...
        else if (pk.HeldItem >= 1231 && pk.HeldItem <= 1251) // lonely mint <> serious mint
        {
            ...
            if (natureEnum)
                pk.Nature = pk.StatNature = (Nature)(int)result;
            else
            {
        ...
        else if (pk.Nickname.Contains("Nat") && typeof(T) == typeof(PA8))
        {
            switch (pk.Nickname.ToLower())
            {
                case string s when s.Contains("adamant"):
                    pk.ClearNickname();
                    pk.Nature = pk.StatNature = Nature.Adamant;
                    break;
                case string s when s.Contains("bold"):
                    pk.ClearNickname();
                    pk.Nature = pk.StatNature = Nature.Bold;
                    break;
                case string s when s.Contains("brave"):
                    pk.ClearNickname();
                    pk.Nature = pk.StatNature = Nature.Brave;
                    break;
                case string s when s.Contains("calm"):
                    pk.ClearNickname();
                    pk.Nature = pk.StatNature = Nature.Calm;
                    break;
                case string s when s.Contains("careful"):
                    pk.ClearNickname();
                    pk.Nature = pk.StatNature = Nature.Careful;
                    break;
                case string s when s.Contains("gentle"):
                    pk.ClearNickname();
                    pk.Nature = pk.StatNature = Nature.Gentle;
                    break;
                case string s when s.Contains("hasty"):
                    pk.ClearNickname();
                    pk.Nature = pk.StatNature = Nature.Hasty;
                    break;
                case string s when s.Contains("impish"):
                    pk.ClearNickname();
                    pk.Nature = pk.StatNature = Nature.Impish;
                    break;
                case string s when s.Contains("jolly"):
                    pk.ClearNickname();
                    pk.Nature = pk.StatNature = Nature.Jolly;
                    break;
                case string s when s.Contains("lax"):
                    pk.ClearNickname();
                    pk.Nature = pk.StatNature = Nature.Lax;
                    break;
                case string s when s.Contains("lonely"):
                    pk.ClearNickname();
                    pk.Nature = pk.StatNature = Nature.Lonely;
                    break;
                case string s when s.Contains("mild"):
                    pk.ClearNickname();
                    pk.Nature = pk.StatNature = Nature.Mild;
                    break;
                case string s when s.Contains("modest"):
                    pk.ClearNickname();
                    pk.Nature = pk.StatNature = Nature.Modest;
                    break;
                case string s when s.Contains("naive"):
                    pk.ClearNickname();
                    pk.Nature = pk.StatNature = Nature.Naive;
                    break;
                case string s when s.Contains("naughty"):
                    pk.ClearNickname();
                    pk.Nature = pk.StatNature = Nature.Naughty;
                    break;
                case string s when s.Contains("quiet"):
                    pk.ClearNickname();
                    pk.Nature = pk.StatNature = Nature.Quiet;
                    break;
                case string s when s.Contains("rash"):
                    pk.ClearNickname();
                    pk.Nature = pk.StatNature = Nature.Rash;
                    break;
                case string s when s.Contains("relaxed"):
                    pk.ClearNickname();
                    pk.Nature = pk.StatNature = Nature.Relaxed;
                    break;
                case string s when s.Contains("sassy"):
                    pk.ClearNickname();
                    pk.Nature = pk.StatNature = Nature.Sassy;
                    break;
                case string s when s.Contains("serious"):
                    pk.ClearNickname();
                    pk.Nature = pk.StatNature = Nature.Serious;
                    break;
                case string s when s.Contains("timid"):
                    pk.ClearNickname();
                    pk.Nature = pk.StatNature = Nature.Timid;
                    break;
            }
        ...
        else if (pk.HeldItem >= 1862 && pk.HeldItem <= 1879 || pk.HeldItem == 2549)
        {
            if (pk is PK9 pk9)
                switch (pk.HeldItem)
                {
