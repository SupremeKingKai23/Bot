...
public abstract class PokeRoutineExecutor<T>(IConsoleBotManaged<IConsoleConnection, IConsoleConnectionAsync> cfg) : PokeRoutineExecutorBase(cfg) where T : PKM, new()
{
        ...
        TradeAbuseSettings AbuseSettings, CancellationToken token)
    {
        ...
        if (previous != null)
        {
            ...
            var cd = isDistribution ? AbuseSettings.DistributionCooldown : AbuseSettings.TradeCooldown;     // Time they must wait before trading again.
            if (cd != 0 && TimeSpan.FromMinutes(cd) > delta)
            {
                var wait = TimeSpan.FromMinutes(cd) - delta;
                poke.Notifier.SendNotification(bot, poke, $"You are still on {(isDistribution ? "Distribution" : "trade")} cooldown, and cannot trade for another {wait.TotalMinutes:F1} minute(s).");
                var msg = $"Found {user.TrainerName}{useridmsg} ignoring the {cd} minute trade cooldown. Last encountered {TrainerName} {delta.TotalMinutes:F1} minutes ago.";
    ...
    public static void LogLGPEUserOT(ulong trainerID, string ot, uint tid, uint sid)
    {
        LGPEOTInfo.TryRegister(trainerID, ot, tid, sid);
    }
