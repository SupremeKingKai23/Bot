...
public class TwitchBot<T> where T : PKM, new()
{
    ...
    public void StartingDistribution(string message)
    {
        Task.Run(async () =>
        {
            ...
            await Task.Delay(1_250).ConfigureAwait(false);
    ...
    private void Client_OnDisconnected(object? sender, OnDisconnectedEventArgs e)
    {
        ...
        Task.Run(() => ReconnectTask(reconnectCancellationTokenSource.Token));
    }
    ...
    private async Task ReconnectTask(CancellationToken cancellationToken)
    {
        ...
        {
            if (reconnectAttempts >= Hub.Config.Twitch.ReconnectAttempts)
            {
                client.Disconnect();
                break;
            }
    ...
    private string HandleCommand(TwitchLibMessage m, string c, string args)
    {
        ...
        switch (c)
        {
            ...
            case "hi":
                var str = Settings.HelloResponse;
                var response = string.Format(str, $"@{m.Username}");
                return response;
    ...
    private void Client_OnWhisperReceived(object? sender, OnWhisperReceivedArgs e)
    {
        ...
        try
        {
            ...
            if (typeof(T) != typeof(PB7))
            {
                ...
                lgcode = TradeSettings.GetRandomLGTradeCode();
            }
            else
            {
                code = 00001234;
                if (Settings.UseDigitCode)
                {
                    var num = int.Parse(msg);
                    lgcode = TwitchCommandsHelper<T>.GetLGPETradeCode(num);
                }
                else
                {
                    lgcode = TwitchCommandsHelper<T>.GetLGPETradeCode(msg);
                }
            }
